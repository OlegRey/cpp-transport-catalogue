...
void JsonReader::FillCatalogue(bus_catalog::Catalogue& catalogue) {
...
void JsonReader::FillStopDistances(bus_catalog::Catalogue& catalogue) const {
    const json::Array& arr = GetBaseRequests().AsArray();
    for (auto& request_stops : arr) {
        const auto& request_stops_map = request_stops.AsDict();
        const auto& type = request_stops_map.at("type"s).AsString();
...
std::tuple<std::string_view, std::vector<const bus_catalog::Stop*>, bool> JsonReader::FillRoute(const json::Dict& request_map, bus_catalog::Catalogue& catalogue) const {
    std::string_view bus_number = request_map.at("name"s).AsString();
    std::vector<const bus_catalog::Stop*> stops;
...
bus_catalog::Router JsonReader::FillRoutingSettings(const json::Node& settings) const {
    bus_catalog::Router routing_settings;
    return bus_catalog::Router{ settings.AsDict().at("bus_wait_time"s).AsInt(), settings.AsDict().at("bus_velocity"s).AsDouble() };
}
