...
namespace renderer {
    ...
    std::vector<svg::Polyline> MapRenderer::GetRouteLines(const std::map<std::string_view, const bus_catalog::Bus*>& buses, const SphereProjector& sp) const {
        std::vector<svg::Polyline> result;
        size_t color_num = 0;
        for (const auto& [bus_number, bus] : buses) {
            if (bus->stops.empty()) continue;
            std::vector<const bus_catalog::Stop*> route_stops{ bus->stops.begin(), bus->stops.end() };
    ...
    std::vector<svg::Text> MapRenderer::GetBusLabel(const std::map<std::string_view, const bus_catalog::Bus*>& buses, const SphereProjector& sp) const {
    ...
    std::vector<svg::Circle> MapRenderer::GetStopsSymbols(const std::map<std::string_view, const bus_catalog::Stop*>& stops, const SphereProjector& sp) const {
    ...
    std::vector<svg::Text> MapRenderer::GetStopsLabels(const std::map<std::string_view, const bus_catalog::Stop*>& stops, const SphereProjector& sp) const {
        std::vector<svg::Text> result;
        svg::Text text;
        svg::Text underlayer;
        for (const auto& [stop_name, stop] : stops) {
    ...
    svg::Document MapRenderer::GetSVG(const std::map<std::string_view, const bus_catalog::Bus*>& buses) const {
        svg::Document result;
        std::vector<geo::Coordinates> route_stops_coord;
        std::map<std::string_view, const bus_catalog::Stop*> all_stops;
