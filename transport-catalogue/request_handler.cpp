...
namespace request_handler {
    ...
    std::optional<RouteInfo> RequestHandler::GetRouteInfo(std::string_view start,
        ...
        return routing.GetRouteInfo(routing.GetRouterByStop(catalogue.GetStop(start))->bus_wait_begin,
            routing.GetRouterByStop(catalogue.GetStop(end))->bus_wait_begin);
    }
    ...
    std::vector<geo::Coordinates> RequestHandler::GetStopsCoordinates(TransportCatalogue& catalogue_) const {
        ...
        for (auto& [busname, bus] : buses) {
            ...
            for (auto& stop : bus->stops) {
                ...
                coordinates.lat = stop->lat;
                coordinates.lng = stop->lng;
