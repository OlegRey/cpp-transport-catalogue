        ...
        for (const auto& item : all_buses) {
            ...
            for (size_t i = 0; i < stops_count; ++i) {
                ...
                for (size_t j = i + 1; j < stops_count; ++j) {
                    ...
                    const double speed_factor = bus_velocity_ * (100.0 / 6.0);

                    stops_graph.AddEdge({ bus_info->number, j - i, stop_ids_.at(stop_from->name) + 1, stop_ids_.at(stop_to->name), static_cast<double>(dist_sum) / speed_factor });

                    if (!bus_info->is_circle) {
                        stops_graph.AddEdge({ bus_info->number, j - i, stop_ids_.at(stop_to->name) + 1, stop_ids_.at(stop_from->name), static_cast<double>(dist_sum_inverse) / speed_factor });
                    }
                }
            }
        }
